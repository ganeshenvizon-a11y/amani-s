/**
 * Menu Section — Hero
 * A tactile editorial collage introducing Amani's South Indian menu.
 * Built with exact architectural parity to the Home Page Hero.
 */
...
const MENU_CARDS = [
  {
    ...
    alt: 'Amani Signature Avakai Chicken Biryani served in traditional copper vessel',
  },
  {
    image: '/media/images/dish-chepala-pulusu.jpg',
    alt: 'Nellore Chepala Pulusu — traditional tangy Andhra fish curry',
  },
  {
    image: '/media/images/thali/south-indian-thali.png',
    alt: 'Amani Grand South Indian Thali featuring authentic regional curries',
  },
  {
    image: '/media/images/signatures/naatu-kodi-pulusu.webp',
    alt: 'Naatu Kodi Pulusu — spicy country chicken curry',
  },
  {
    image: '/media/images/signatures/bhimavaram-mixed-pulav.webp',
    alt: 'Bhimavaram Mixed Pulav with fragrant rice and slow-cooked spices',
  },
  {
    image: '/media/images/signatures/gongura-mutton-curry.webp',
    alt: 'Gongura Mutton Curry — tender mutton cooked with sorrel leaves',
  },
];
...
const MENU_CARD_META = [
  { stamp: 'AVAKAI BIRYANI', tag: 'SIGNATURE DUM' },
  { stamp: 'CHEPALA PULUSU', tag: 'COASTAL CATCH' },
  { stamp: 'ROYAL THALI', tag: 'HERITAGE FEAST' },
  { stamp: 'NAATU KODI', tag: 'COUNTRY CHICKEN' },
  { stamp: 'BHIMAVARAM PULAV', tag: 'SPECIAL RICE' },
  { stamp: 'GONGURA MUTTON', tag: 'ANDHRA CLASSIC' },
];
...
const DYNAMIC_WORDS = ['Memory', 'Tradition', 'Heritage'] as const;
...
export function Hero() {
  ...
  // Dynamic word animation state for Memory -> Tradition -> Heritage
  ...
  if (cardRotations.current.length === 0) {
    ...
    cardScales.current = MENU_CARDS.map(() => 0.95 + Math.random() * 0.13);
  }
  ...
  return (
    <section ref={heroRef} className="menu-hero menu-hero--editorial" aria-label="Amani's Culinary Menu">
      <div className="menu-hero__bg-pattern" aria-hidden="true">
        <img
          src="/media/images/hero-pattern.png"
          alt=""
          className="menu-hero__bg-pattern-img"
          loading="eager"
          decoding="async"
        />
      </div>
      <div className="menu-hero__content">
        <p className="menu-hero__kicker">South Indian Culinary Heritage / Handcrafted Daily</p>
        <h1 className="menu-hero__title" data-split="true">
          <span className="menu-hero__title-line">
            <span className="menu-hero__title-line-inner">
              Recipes from <span className="menu-hero__word menu-hero__word--spice">Fire</span>.
            </span>
          </span>
          <span className="menu-hero__title-line">
            <span className="menu-hero__title-line-inner">
              Flavour from{' '}
              <span className="menu-hero__dynamic-wrap">
                <span className={`menu-hero__word menu-hero__word--dynamic is-${wordState}`}>
                  {DYNAMIC_WORDS[wordIndex]}
                </span>
                <span className="menu-hero__period">.</span>
              </span>
            </span>
          </span>
        </h1>
      ...
      <div className="menu-hero__card-deck" aria-label="Draggable Amani signature dishes visual introduction">
        {MENU_CARDS.map((card, index) => {
          const meta = MENU_CARD_META[index] || { stamp: 'AMANI SPECIAL', tag: 'SIGNATURE' };
          ...
          return (
            <button
              ...
              className={`menu-hero__card menu-hero__card--retro menu-hero__card--tear-${tearVariant} ${draggedCard === index ? 'is-grabbing' : ''}`}
              ...
              aria-label={`Open or drag signature dish card: ${card.alt}`}
            >
              {/* Translucent Frosted Scotch Tape Strip */}
              <span className="menu-hero__card-tape" aria-hidden="true" />
              
              {/* Paper Crease Line */}
              <span className="menu-hero__card-crease" aria-hidden="true" />

              {/* Vintage Ink Stamp */}
              <span className="menu-hero__card-stamp" aria-hidden="true">
                {meta.stamp}
              </span>

              {/* Photo Frame & Vintage Vignette */}
              <div className="menu-hero__card-media">
                <img src={card.image} alt="" loading={index < 3 ? 'eager' : 'lazy'} decoding="async" />
                <div className="menu-hero__card-vignette" aria-hidden="true" />
              </div>

              {/* Retro Footer Tag & Number */}
              <div className="menu-hero__card-footer">
                <span className="menu-hero__card-tag">{meta.tag}</span>
                <span className="menu-hero__card-caption">{String(index + 1).padStart(2, '0')}</span>
              </div>
            </button>
          );
        })}
      ...
      {selectedCard !== null && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`Expanded dish image: ${MENU_CARDS[selectedCard].alt}`}
          onPointerDown={(event) => {
            if (event.target === event.currentTarget) setSelectedCard(null);
          }}
        >
          <figure className="hero-image-lightbox__content">
            <img src={MENU_CARDS[selectedCard].image} alt={MENU_CARDS[selectedCard].alt} />
            <figcaption>{MENU_CARD_META[selectedCard].tag} — {MENU_CARD_META[selectedCard].stamp}</figcaption>
